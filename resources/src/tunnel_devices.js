...
function copy_script(id, secret) {
  var clipboard = new ClipboardJS(`#copy_${id}`, {
    text: function () {
      return `bash <(curl -sSL "https://gitlab.com/ProjectPF/PortForwardGo/-/raw/master/scripts/install_securetunnel.sh") --api ${window.location.origin}/sync/tunnel_device --id ${id} --secret ${secret}`;
    }
  })
