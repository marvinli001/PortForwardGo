...
$("#adds_protocol").on("change", function () {
  ...
  switch (protocol) {
    case "http": case "https":
      $("#tag_adds_bind").text("绑定域名");
      $("#adds_bind").prop("placeholder", "example.com");
      break;
    default:
      $("#tag_adds_bind").text("监听端口");
    ...
    .done(function (response) {
      if (response.Ok) {
        if (response.Data == null) {
          $("#rule_usage").text("规则 0 / " + user.rule + " 条")
          mdui.updateTables("#rule_table");
          return;
        }

        $("#rule_usage").text("规则 " + response.Data.length + " / " + user.rule + " 条")
    ...
    .done(function (response) {
      if (response.Ok) {
        ...
        for (i in response.Data) {
          ...
          $("#adds_dest").append(`<option data-type="device" value="${device.id}">${escapeHTML(device.name)}</option>`);
        }
