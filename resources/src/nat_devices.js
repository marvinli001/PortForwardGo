...
function copy_script(id, secret) {
  var clipboard = new ClipboardJS(`#copy_${id}`, {
    text: function () {
      return `bash <(curl -sSL "https://gitlab.com/ProjectPF/PortForwardGo/-/raw/master/scripts/install_rclient.sh") --api ${window.location.origin}/sync/nat_device --id ${id} --secret ${secret}`;
    }
  })
