...
function copy_script(id, secret) {
  ...
  if (settings.license == "") {
    command = `bash <(curl -sSL "https://gitlab.com/ProjectPF/PortForwardGo/-/raw/master/scripts/install.sh") --api ${window.location.host} --secret ${secret}`
  } else {
    command = `bash <(curl -sSL "https://gitlab.com/ProjectPF/PortForwardGo/-/raw/master/scripts/install.sh") --api ${window.location.host} --secret ${secret} --license ${settings.license}`
  }
